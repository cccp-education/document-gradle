...
class MarkdownReleaseNotesRendererTest {

    private val renderer = MarkdownReleaseNotesRenderer()
    ...
    @Test
    fun `render produces h1 title with version`() {
    ...
    @Test
    fun `render groups commits by category as h2 sections`() {
        val commits = listOf(
    ...
    fun `renderToFile writes content to file and returns it`(@TempDir dir: File) {
        val output = dir.resolve("release-notes.md")
    ...
    fun `renderToFile creates parent directories`(@TempDir dir: File) {
        val output = dir.resolve("build/release-notes/v1.md")
