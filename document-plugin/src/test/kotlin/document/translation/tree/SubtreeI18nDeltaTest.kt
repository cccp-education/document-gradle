...
class SubtreeI18nDeltaTest {
    ...
    private fun sampleTree(): SiteTree {
        val ab = Article(path = "contents/ab-partition")
        val cd = Article(path = "contents/cd-partition")
        val contents = Section(path = "contents", articles = listOf(ab, cd))
        val blog = Section(path = "blog", articles = listOf(Article(path = "blog/hello")))
        return SiteTree(Site(path = "", sections = listOf(contents, blog)))
    }
    ...
    fun `delta is empty when no source changed`() {
        val tree = sampleTree()
        val checksums = mapOf(
            "contents/ab-partition" to "hash-ab-v1",
            "contents/cd-partition" to "hash-cd-v1",
    ...
    fun `delta detects a single modified article in the whole tree`() {
        ...
        val after = before.toMutableMap().apply { this["contents/ab-partition"] = "hash-ab-v2" }
        val planner = SubtreeI18nPlanner(tree, before)

        val delta = planner.computeDelta(after)

        assertEquals(1, delta.modifiedArticles.size)
        assertEquals("contents/ab-partition", delta.modifiedArticles[0].path)
    ...
    fun `delta for a subtree only includes modified articles within that subtree`() {
        ...
        val after = before.toMutableMap().apply {
            this["contents/ab-partition"] = "hash-ab-v2"
            this["blog/hello"] = "hash-blog-v2"
        }
        val planner = SubtreeI18nPlanner(tree, before)

        val delta = planner.computeDelta(after, subtreePath = "contents")
    ...
    fun `integrity outside subtree is preserved - blog article not in delta`() {
        ...
        assertTrue(delta.modifiedArticles.isEmpty())
        assertEquals(2, delta.untouchedArticles.size)
        assertTrue(delta.untouchedArticles.any { it.path == "contents/ab-partition" })
    }
    ...
    fun `delta carries translatable segment count per modified article`() {
        val tree = SiteTree(Site(path = "", sections = listOf(
            Section(path = "contents", articles = listOf(
                Article(path = "contents/ab-partition", content = content("Hello world")),
                Article(path = "contents/cd-partition", content = content("Bonjour le monde"))
            ))
        )))
        val before = mapOf(
            "contents/ab-partition" to "hash-ab-v1",
        ...
        assertEquals("contents/ab-partition", mod.path)
    ...
    fun `new article not in before checksums is included as modified`() {
        ...
        val after = before.toMutableMap().apply { this["contents/ab-partition"] = "hash-ab-new" }
        val planner = SubtreeI18nPlanner(tree, before)
    ...
    fun `deleted article checksum in before but not in after is not a modification`() {
        val tree = SiteTree(Site(path = "", sections = listOf(
            Section(path = "contents", articles = listOf(
                Article(path = "contents/cd-partition")
            ))
        )))
        val before = mapOf(
            "contents/ab-partition" to "hash-ab-v1",
        ...
        val after = mapOf("contents/cd-partition" to "hash-cd-v1")
        val planner = SubtreeI18nPlanner(tree, before)
    ...
    fun `backward compat - no tree falls back to flat walk of all articles`() {
        val articles = listOf(
            Article(path = "contents/ab-partition"),
            Article(path = "contents/cd-partition"),
